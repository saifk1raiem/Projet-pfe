...
export function ToolsPage({ currentUser, accessToken }) {
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.detail || tr("Erreur chatbot.", "Chatbot error."));
      }

      const assistantReply = String(data?.reply ?? "").trim();
      if (!assistantReply) {
        throw new Error(tr("Reponse vide du chatbot.", "Empty chatbot response."));
      }
      ...
      setChatError(error?.message || tr("Erreur chatbot.", "Chatbot error."));
    } finally {
      ...
      <section className="min-h-full bg-[radial-gradient(circle_at_top,_rgba(122,224,148,0.14),_transparent_34%),linear-gradient(180deg,#f7faf8_0%,#eff6f1_100%)] px-6 py-8 md:px-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-6">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#607066]">
                  {tr(
                    "Cette page accueille des outils internes pour vous aider au quotidien.",
                    "This page hosts internal tools to help your daily workflow.",
                  )}
                </p>
              </div>
              <Badge className="rounded-full bg-[#edf9f3] px-3 py-1 text-[#157347]">
                {tr("2 outils actifs", "2 active tools")}
              </Badge>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            <ToolCard
              icon={Bot}
              title={tr("Chatbot", "Chatbot")}
              description={tr(
                "Posez vos questions et obtenez des reponses assistees par IA directement dans l'application.",
                "Ask questions and get AI-assisted answers directly in the app.",
              )}
              badge={tr("Disponible", "Available")}
              onClick={() => setActiveTool(TOOL_IDS.chatbot)}
            />
            <ToolCard
              icon={Sparkles}
  ...
  if (activeTool === TOOL_IDS.chatbot) {
    return (
      <section className="min-h-full bg-[radial-gradient(circle_at_top,_rgba(122,224,148,0.14),_transparent_34%),linear-gradient(180deg,#f7faf8_0%,#eff6f1_100%)] px-6 py-8 md:px-8">
        <div className="mx-auto flex max-w-5xl flex-col gap-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="leoni-display-lg text-3xl font-semibold text-[#1d2a21] md:text-4xl">
                  {tr("Chatbot IA", "AI Chatbot")}
                </h1>
                <p className="mt-2 text-sm leading-6 text-[#607066]">
                  {tr(
                    "Discutez avec l'assistant IA pour obtenir de l'aide rapide.",
                    "Chat with the AI assistant to get quick help.",
                  )}
                </p>
              </div>
            </div>
            <Badge className="rounded-full bg-[#eef5ff] px-3 py-1 text-[#0f63f2]">
              {tr("Connexion securisee", "Secured connection")}
            </Badge>
          </div>

          <Card className="rounded-[28px] border border-[#dce8df] bg-white/92 p-6 shadow-[0_22px_60px_rgba(24,48,37,0.08)]">
            <div className="mb-4 flex items-center gap-2 text-[#1f2d24]">
              <Bot className="h-5 w-5 text-[#0f63f2]" />
              <p className="text-base font-semibold">{tr("Conversation", "Conversation")}</p>
            </div>

            <div className="h-[420px] overflow-y-auto rounded-2xl border border-[#e4ece7] bg-[#f7faf8] p-4">
              {chatMessages.length ? (
                <div className="space-y-3">
                  {chatMessages.map((message) => (
                    <div
                      ...
                      className={cn(
                        ...
                        message.role === "user"
                          ? "ml-auto bg-[#005ca9] text-white"
                          : "bg-white text-[#24342a] border border-[#e4ece7]",
                      )}
                    >
                      {message.content}
                    </div>
                  ...
                  {isSendingChat ? (
                    <div className="max-w-[90%] rounded-2xl border border-[#e4ece7] bg-white px-4 py-3 text-sm text-[#5f6d66]">
                      <span className="inline-flex items-center gap-2">
                        <LoaderCircle className="h-4 w-4 animate-spin" />
                        {tr("Le chatbot ecrit...", "Chatbot is typing...")}
                      </span>
                    </div>
                  ) : null}
                </div>
              ) : (
                <p className="text-sm text-[#65736a]">
                  {tr("Aucun message pour le moment. Posez votre premiere question.", "No messages yet. Ask your first question.")}
                </p>
              )}
            </div>

            {chatError ? (
              <div className="mt-4 rounded-2xl border border-[#f4c8c8] bg-[#fff4f4] px-4 py-3 text-sm text-[#b03131]">
                {chatError}
              </div>
            ) : null}
            ...
            <div className="mt-4 flex flex-col gap-3">
              <Textarea
                ...
                rows={4}
                placeholder={tr("Ecrivez votre message...", "Write your message...")}
                className="rounded-2xl border-[#d5dce0] bg-white"
              />
