...
export function FormationPage({ openFormationId = null, currentUser, accessToken }) {
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <FormationStat icon={BookOpen} title={tr("Formations", "Trainings")} value={totalFormations} color={{ bg: "bg-[#e6f0ff]", text: "text-[#0f63f2]" }} />
        <FormationStat icon={Clock3} title={tr("Duree moyenne", "Average duration")} value={tr(`${averageDuration} jours`, `${averageDuration} days`)} color={{ bg: "bg-[#e8f1fb]", text: "text-[#005ca9]" }} />
        <FormationStat icon={Layers3} title={tr("Domaines", "Domains")} value={domainesCount} color={{ bg: "bg-[#f3edff]", text: "text-[#9029ff]" }} />
        <FormationStat icon={CheckCircle2} title={tr("Avec duree", "With duration")} value={configuredDurations.length} color={{ bg: "bg-[#eff2f5]", text: "text-[#5f6777]" }} />
      </div>
