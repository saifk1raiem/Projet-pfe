...
function buildAdvisorFindings(collaborateurs, formations) {
  ...
  const indectionCollaborateurs = collaborateurs.filter((collab) => collab.phase === "indection");
  if (indectionCollaborateurs.length > Math.max(formations.length * 5, 10)) {
    findings.push({
      id: "advisor-capacity-warning",
      severity: "warning",
      source: "formation",
      title: "Charge d'integration elevee",
      description: `${indectionCollaborateurs.length} collaborateurs sont en indection. Verifiez si les formateurs disponibles sont suffisants pour absorber ce volume.`,
    });
  }
    ...
    const loadAdvisorData = async () => {
      ...
      try {
        const [qualificationResponse, formationsResponse] = await Promise.all([
          fetch("http://127.0.0.1:8000/api/v1/qualification", {
            headers: { Authorization: `Bearer ${accessToken}` },
          }),
          fetch("http://127.0.0.1:8000/api/v1/formations", {
            headers: { Authorization: `Bearer ${accessToken}` },
          }),
        ]);
